...
# 查看表倾斜率的SQL
table_percentage_ori=$bashpath"/table-percentage-sql/table-percentage.sql-ori"
table_percentage=$bashpath"/table-percentage-sql/table-percentage.sql"

# 所产生的日志路径
percentage_log=$bashpath"/log"

#创建生成结果的临时目录与倾斜的详细目录
temp_all_table_results=$percentage_log"/"$currentDate"/temp-percentage-results"
table_percentage_log=$percentage_log"/"$currentDate"/table-percentage"
...
if [ -d $percentage_log ];then
     rm -rf $percentage_log
fi


mkdir -p  $temp_all_table_results
mkdir -p  $table_percentage_log

# 根据schema获取schema下的表,汇总成一个文件
array=(${schema_inspect//,/ })
for schema_var in ${array[@]}
do
   cp $table_percentage_sql_ori $table_percentage_sql
   sed -i 's/schemaName/'$schema_var'/g' $table_percentage_sql
   psql -d $gpdatabase  -h $gpip -p $gpport -U $gpuser -f  $table_percentage_sql >> $temp_all_table_results/$currentDate-$schema_var".txt"   
...
do
   ...
   cp $table_percentage_ori $table_percentage
   sed -i 's/tableName/'$tablename'/g' $table_percentage
   percent_result=`psql -d $gpdatabase  -h $gpip -p $gpport -U $gpuser -f  $table_percentage`
   ...
   if [[ $percent_number -ge $percent_hidden ]] ;then
       ...
       echo $tablename","$maxnumber","$minnumber","$percent_number","$orTableSize","$tableDistributed >> $table_percentage_log/$currentDate"-temp.csv"
   fi

   # 删除倾斜率的文件,以便下次使用
  if [ -f $table_percentage ];then
   rm  -rf $table_percentage
  fi
...
#  判断是否有满足表的信息
finish_file_temp=$table_percentage_log/$currentDate"-temp.csv"
finish_file=$table_percentage_log/$currentDate"-finish.csv"
